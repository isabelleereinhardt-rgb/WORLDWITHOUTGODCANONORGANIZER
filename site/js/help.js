...
const FAQS = [
  ...
  { needs: "assistant", topic: "Assistant", q: "Does the assistant invent things?",
    a: "No. It answers only from your own entries and lists the ones it used underneath. Out of the box it runs entirely on this device: it searches, summarizes, cross-checks and answers from your words, with no request leaving the browser. If you connect your own API key in Settings, the entries that match a question are sent to that model to answer it; the chip at the top of the assistant always names which of the two you are getting." },
  { needs: "assistant", topic: "Assistant", q: "Can I connect a real AI model?",
    a: "Yes, with your own key, in Settings under Assistant. The local search still runs first and decides which entries are relevant; those passages are then sent to the model with your question, so answers come back reasoned rather than assembled, with the same citations. Your key is kept on this device, is never uploaded, and is deliberately left out of backups. If a request fails you still get the on-device answer, with the reason why." },
